  ...
  if (setupUrl) {
    return (
      <div className="animate-slide-up text-center py-4 space-y-4">
        ...
        <p className="text-green-400 font-semibold text-sm">Member created! Password setup email sent.</p>
        {setupUrl && (
          <div className="bg-slate-800 border border-slate-700 rounded-xl p-3 text-left">
            <p className="text-xs text-slate-500 mb-1">DEV — Setup link:</p>
            <a href={setupUrl} target="_blank" rel="noreferrer"
              className="text-xs text-cyan-400 break-all hover:underline">{setupUrl}</a>
          </div>
        )}
