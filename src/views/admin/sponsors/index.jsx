...
function SponsorViewModal({ sponsor, onClose, onEdit }) {
  ...
  return createPortal(
    <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/75 backdrop-blur-sm px-4 py-8" onClick={onClose}>
      <div className="sp-card-enter sp-royal-glow relative bg-slate-900 border border-amber-400/30 rounded-3xl w-full max-w-md shadow-2xl overflow-hidden max-h-[90vh] overflow-y-auto"
        ...
        <div className="relative flex items-center justify-between px-5 py-4 border-b border-white/5">
          ...
          <button type="button" onClick={onClose}
            className="w-8 h-8 rounded-full bg-white/10 hover:bg-white/20 flex items-center justify-center text-slate-300 hover:text-white transition-colors">×</button>
        </div>
        ...
        <div className="relative p-5 space-y-4">
          ...
          <div className="rounded-xl p-4 text-center"
            ...
            <p className="text-3xl font-black" style={{ color: "#fbbf24", textShadow: "0 0 16px rgba(245,158,11,0.5)", animation: "countUp 0.5s cubic-bezier(0.34,1.4,0.64,1) both" }}>
              {fmt(sponsor.amount)}
            </p>
          </div>
...
export default function AdminSponsors() {
  ...
  return (
    <TripModuleShell title="Sponsors" description="Manage trip sponsors and additional funding" loading={loading && !!selectedTripId}>
      ...
      {selectedTripId && (
        <>
          {/* ── Sponsor total — royal hero banner ── */}
          {items.length > 0 && (
            <div className="sp-royal-glow mb-4 relative rounded-3xl overflow-hidden border border-amber-400/30"
              ...
              <div className="px-5 py-5 flex items-center justify-between gap-4 relative">
                <div>
                  ...
                  <p className="text-[11px] text-slate-400 mt-1.5">{items.length} sponsor{items.length !== 1 ? "s" : ""} backing this trip</p>
                </div>
            ...
            <ul className="space-y-3">
              {[...items]
                ...
                .map((sp, i) => {
                  ...
                  return (
                    <li key={sp._id} className="sp-card-in relative rounded-2xl px-4 py-3.5 border transition-all duration-300 hover:-translate-y-0.5"
                      style={{
                    ...
                    >
                      {/* Top row — ranked avatar + name/notes (left) + action icons (top-right) */}
                      <div className="flex items-start justify-between gap-2">
                        ...
                        <div className="flex items-center gap-1.5 shrink-0">
                          <button type="button" onClick={() => setViewSp(sp)} title="View sponsor"
                            className="w-8 h-8 flex items-center justify-center rounded-lg bg-slate-800 border border-slate-700 text-slate-400 hover:text-violet-300 hover:border-violet-700/50 transition-colors">
                            <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path strokeLinecap="round" strokeLinejoin="round" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.477 0 8.268 2.943 9.542 7-1.274 4.057-5.065 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                          </button>
                          <button type="button" onClick={() => setEditSp(sp)} title="Edit sponsor"
                          ...
                          {isSuperAdmin && (
                            <button type="button" onClick={() => handleDelete(sp)} title="Delete sponsor"
                              className="w-8 h-8 flex items-center justify-center rounded-lg bg-red-950/40 border border-red-800/50 text-red-400 hover:bg-red-900/60 hover:text-red-300 transition-colors">
